...
const checkPosition = async () => {
    try {
        ...
        if (!isInRange) {
            notifyTelegram(`Position https://pancakeswap.finance/liquidity/position/infinityCl/${positionId}?chain=bsc&persistChain=1  is out of range ${currentTick < position.tickUpper ? 'break Upper Range ' : 'break Lower Range'}  currentTick: ${currentTick}`, process.env.TELEGRAM_TOKEN, process.env.TELEGRAM_CHAT)
        }
    } catch (error) {
